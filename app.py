...
# ✅ Dono files ke paths define kiye
    ...
    if os.path.exists(file_path):
        try:
            with open(file_path, 'r') as f:
                data = json.load(f)
                # List se saare "token" values nikal rahe hain
...
# ✅ Tweak/Encryption Functions (No changes here)
...
# ✅ Profile Visit Ke Liye Request (visit_ind.json ka token use karega)
...
# ✅ Likes Send Karne Ke Liye Request (token_ind.json ka token use karega)
...
# ✅ Async send likes (token_ind.json ke saare tokens ke sath)
...
def like_handler():
    ...
    try:
        # 1️⃣ Profile Visit ke liye visit_ind.json se tokens load karein
        ...
        # 2️⃣ Likes bhejane ke liye token_ind.json se tokens load karein
        like_tokens = get_tokens_from_file(LIKES_TOKENS_FILE)
        if not like_tokens:
            return jsonify({"error": "No valid like tokens found in token_ind.json"}), 401

        # Player info check karne ke liye visit_ind.json ka pehla token use hoga
        enc_uid = encrypt_message(create_uid_proto(uid))
        before = make_request(enc_uid, visit_tokens[0])
        if not before:
        ...
        # 3️⃣ Asynchronous likes send karne ka process (using like_tokens)
        responses = asyncio.run(send_likes(uid, like_tokens))
        success_count = sum(1 for r in responses if r == 200)

        # Likes ke baad ka data check (again using visit_tokens)
        after = make_request(enc_uid, visit_tokens[0])
...
@app.route('/')
def home():
    return jsonify({"status": "online", "message": "Like API is running via Multi-JSON (Visit & Like Separate) ✅"})
